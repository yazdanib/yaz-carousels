...
- **The cover headline fills roughly 75% of the frame, but only when it's text-only.** If a cover slide has a `background_image`, it automatically switches to the Style B look instead: the photo sits smaller with rounded corners inside a margin, display name top-left in that margin, handle bottom-left, swipe hint bottom-right, headline overlaid on the photo (upper-left, with a scrim behind it for legibility) rather than centred and huge. No spec flag needed, this is automatic based on whether `background_image` is set on the cover slide. Keep the cover headline short either way, the image version has less room, not more (default 80px vs. 140px).
